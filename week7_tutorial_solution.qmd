---
title: "ETC1010/5510 Tutorial 7 - Solution"
...
## 🚧 Exercise 6A: Understanding joins
...
In the first line of the code chunk above, we perform a left join between `gap_co2_au` and `gap_life_au.` A left join will use `gap_co2_au` as the master table to which the `gap_life_au` records are joined. So, because `gap_co_2` only has 3 rows, this limits the number of rows that `gap_life_au` can be joined to. Only the rows in `gap_life_au` from the years 2012, 2013 and 2014 will be able to be joined. 
...
## ✈️ Exercise 6B: Mapping data with joins (flights movements)
...
## 🛫 Exercise 6C: Data wrangling with joins 🛬
...
When the wind is reasonably strong (>20mph), and if it is blowing across the runway, there may be some delays.
